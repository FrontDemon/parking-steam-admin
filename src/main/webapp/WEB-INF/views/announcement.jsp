...
<section class="Hui-article-box">
    <nav class="breadcrumb"><i class="Hui-iconfont"></i>
        ...
        <span class="c-999 en">&gt;</span><span class="c-666">信息发布管理</span>
        <span class="c-999 en">&gt;</span><span class="c-666">公告管理</span>
    ...
    <div class="Hui-article">
        <article class="cl pd-20">
            ...
            <div style="text-align: center">
                <input class="input-text radius" style="width: 200px" type="text" name="key" id="key" placeholder="输入标题"/>
                <button type="submit" class="btn btn-success" id="search-btn" name="search-btn"><i class="Hui-iconfont">&#xe665;</i> 搜索公告</button>
            </div>
            <div class="cl pd-5 bg-1 bk-gray mt-20" >
                <span class="l">
				<a href="javascript:;" onclick="notice_add('添加公告','${APP_PATH}/infoManagement/notice-add','800','300')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加公告</a></span>
                <!--  <p style="display: inline-block">共有数据：<span id="noticeNumber">0</span>条</p>-->
                <span class="r">共有数据：<strong id="noticeNumber">0</strong> 条</span>
            </div>
            <table class="table table-border table-bordered table-hover table-bg">
                <caption class="bg-1" style="height: 40px;line-height: 40px;">公告列表</caption>
                <thead>
                <tr class="text-c">
                    <th>序号</th>
                    <th>标题</th>
...
<script type="text/javascript">
	$(function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				$("#noticeNumber").html(dataList.length);
				for(var i=0;i<dataList.length;i++){
					var tr = $("<tr class='text-c'></tr>");
					$("#tbody").append(tr);
					var td1 = $("<td>"+(i+1)+"</td>");
					tr.append(td1);
					var td2 = $("<td>"+dataList[i].title+"</td>");
					tr.append(td2);
					var td3 = $("<td>"+dataList[i].context+"</td>");
					tr.append(td3);	
					var td4 = $("<td>"+formatDate(dataList[i].createTime)+"</td>");
					tr.append(td4);
					var td5 = $("<td>"+dataList[i].admin.name+"</td>");
					tr.append(td5);
					var td6 = $("<td><a class='ml-5' href='javascript:;' style='text-decoration:none' title='编辑' onclick='notice_edit(this)'><i class='Hui-iconfont'>&#xe60c;</i></a><a class='ml-5' href='javascript:;' style='text-decoration:none' title='删除' onclick='notice_del(this)'><i class='Hui-iconfont'>&#xe609;</i></a></td>");
					tr.append(td6);
		...
		$("#search-btn").on("click",function(){
			var title = $("#key").val();
			$.ajax({
				url:"${APP_PATH}/infoManagement/searchAnnouncement",
				data:{"title":title},
				type:"GET",
				success:function(data){
					console.log(data);
	...
	function notice_edit(obj){
		var noticeId = $(obj).parents("tr").find(".inp").val();
		var urltmp = "${APP_PATH}/infoManagement/notice-edit?id="+noticeId;
		layer_show('编辑公告',urltmp,'800','300');
	}
	...
	function notice_del(obj){
		layer.confirm('确认要删除吗？',function(index){
			...
			$.ajax({
				type: 'POST',
				url: '${APP_PATH}/infoManagement/deleteAnnouncement',
				data:{"id":noticeId},
				dataType: 'json',
				success: function(data){
					if(data.code==2){
